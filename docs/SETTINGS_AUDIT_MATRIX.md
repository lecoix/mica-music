...
> 现状复核：2026-09-11。设置页已完成一轮精简/分组重构，并新增启动自动播放、迷你栏滑动关闭、歌曲副行 HQ/SQ/HR、Lyricon、USB/远端相关入口；矩阵后续新增项继续要求“持久化 owner + runtime consumer”双向可追踪。
...
| 启动时自动播放 | `PlaybackUiPreferences` / `auto_play_on_launch` | `PlaybackRuntime` 在本次进程首次出现可播当前曲后请求 `play()`；Service 恢复队列仍为暂停 | `ACTIVE` | 默认关闭；外部点播优先；回到前台不重复触发 |
