...
### 主界面与设置
- [x] 侧栏导航：**歌曲 / 歌手 / 专辑 / 最近 / 我喜欢 / 歌单 / 音乐库分析 / 设置**
- [x] **搜索**：标题 / 艺术家 / 专辑 / 文件名
...
### 主界面 · 迷你播放器底栏
- [ ] **底栏封面被圆角裁切**：主页 `MiniPlayer` 左侧 `SongCover`（`coverXs`）贴近屏幕底边时，四角（尤其左下）被 **系统圆角屏 / 手势导航区** 或父级裁剪挡住，与设计规范 **0dp 直角封面** 不符。
  - **排查**：`MainActivity` `Scaffold` 与 `HomeScreen` `contentPadding`（`navigationBars`）是否重复或不足；`MiniPlayer` 是否需单独 `navigationBars` + 左右/底 **safeDrawing** 内边距。
  - **修复方向**：底栏整体上移或左右留白，使封面完全落在安全矩形内；封面保持直角、不改为圆角蒙版；真机（圆角屏 + 手势条）验收。
