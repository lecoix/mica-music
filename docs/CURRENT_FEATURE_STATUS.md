...
> 最后更新：2026-08-21。本文记录近期已经接入代码的功能，以及“代码完成”和“真机验收完成”之间的边界。实现细节仍以当前源码和测试为准。
...
| USB Exclusive Hybrid 设置子页 | “音频与设备”提供独立 USB 子页，按 DAC/实际输出、显式输出模式、独占事实、传输状态、支持与诊断分区。页面只读取 owner facts 显示 ACTIVE/格式/epoch/session，不以偏好推断活动状态；仅开放严格匹配的 SK02，Native 始终标记实验。 | 导航、搜索和 presentation JVM 测试覆盖；页面遵循直角、留白分组、纯字色选项、文字状态和 48dp 动作规范。Redmi 22081212C 已检查首屏、滚动到底、动作可达和系统返回；TalkBack 仍未人工验收。这条 UI 状态不等同于重新取得 USB 音频资格证据。 |
