...
1. **队列页 / 队列切换**：竖屏嵌入播放页；封面与歌词聚焦共用 `headerFocus`；底栏进度与五键收起；点选播放不关闭。横屏仍用侧栏。
...
- [ ] 竖屏队列：封面缩至歌词顶栏、列表嵌入腾出空间、底栏进度与五键收起、点选不关闭、返回关闭；横屏仍侧栏
- [ ] 封面底边进度模式 ↔ 歌词页返回无跳变
- [ ] 原样比例横/竖封面无两步位移
