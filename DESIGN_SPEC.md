...
| 设置 · 音频与设备 | ✅ | `AUDIO`：ReplayGain、启动时自动播放（默认关闭）、音频焦点、独立「音效实验室」页（宽度/音色/混响/360° 环绕，默认关闭），以及独立的“USB 独占输出”子页。USB 子页沿用设置行、选项行、动作行与提示行，不使用 Card、圆形状态点或独立边框；活动状态只读取 owner facts |
