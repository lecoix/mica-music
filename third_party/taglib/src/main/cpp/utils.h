...
jclass trackProbeClass = nullptr;
jmethodID trackProbeConstructor = nullptr;
...
extern "C" JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *) {
    ...
    audioPropertiesConstructor = env->GetMethodID(audioPropertiesClass, "<init>", "(IIIII)V");

    jclass _trackProbeClass = env->FindClass("com/kyant/taglib/TrackProbe");
    trackProbeClass = reinterpret_cast<jclass>(env->NewGlobalRef(_trackProbeClass));
    env->DeleteLocalRef(_trackProbeClass);
...
extern "C" JNIEXPORT void JNI_OnUnload(JavaVM *vm, void *) {
    ...
    env->DeleteGlobalRef(trackProbeClass);
    ...
    trackProbeClass = nullptr;
    trackProbeConstructor = nullptr;
...
static jint extractBitsPerSample(TagLib::File *file) {
    if (file == nullptr) {
        return 0;
    }

    if (auto *flac = dynamic_cast<TagLib::FLAC::File *>(file)) {
        if (const auto *properties = flac->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *oggFlac = dynamic_cast<TagLib::Ogg::FLAC::File *>(file)) {
        if (const auto *properties = oggFlac->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *wav = dynamic_cast<TagLib::RIFF::WAV::File *>(file)) {
        if (const auto *properties = wav->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *aiff = dynamic_cast<TagLib::RIFF::AIFF::File *>(file)) {
        if (const auto *properties = aiff->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *mp4 = dynamic_cast<TagLib::MP4::File *>(file)) {
        if (const auto *properties = mp4->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *asf = dynamic_cast<TagLib::ASF::File *>(file)) {
        if (const auto *properties = asf->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *ape = dynamic_cast<TagLib::APE::File *>(file)) {
        if (const auto *properties = ape->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *wavPack = dynamic_cast<TagLib::WavPack::File *>(file)) {
        if (const auto *properties = wavPack->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *dsf = dynamic_cast<TagLib::DSF::File *>(file)) {
        if (const auto *properties = dsf->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *dsdiff = dynamic_cast<TagLib::DSDIFF::File *>(file)) {
        if (const auto *properties = dsdiff->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    } else if (auto *matroska = dynamic_cast<TagLib::Matroska::File *>(file)) {
        if (const auto *properties = matroska->audioProperties()) {
            return static_cast<jint>(properties->bitsPerSample());
        }
    }
...
jobject getAudioProperties(JNIEnv *env, const TagLibExt::FileRef &f) {
    ...
    const jint bitsPerSample = extractBitsPerSample(f.file());
    if (audioProperties) {
