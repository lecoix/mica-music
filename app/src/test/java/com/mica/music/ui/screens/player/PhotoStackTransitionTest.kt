...
class PhotoStackTransitionTest {
    ...
    @Test
    fun burnPolicyRequiresEnabledMotionAndStableScene() {
        assertEquals(
            false,
            shouldUsePhotoStackBurn(
                enabled = false,
                motionEnabled = true,
                stableScene = true,
    ...
    @Test
    fun burnPolicySkipsRapidRepeatedSwitches() {
        assertEquals(
            false,
            shouldUsePhotoStackBurn(
                enabled = true,
                motionEnabled = true,
                stableScene = true,
                elapsedSincePreviousMs = PhotoStackRapidSkipWindowMs - 1,
                randomValue = 0f,
            ),
        )
    }
    ...
    @Test
    fun burnPolicyUsesProbabilityThreshold() {
        assertEquals(
            true,
            shouldUsePhotoStackBurn(
                enabled = true,
                motionEnabled = true,
                stableScene = true,
                elapsedSincePreviousMs = PhotoStackRapidSkipWindowMs,
                randomValue = PhotoStackBurnProbability - 0.001f,
            ),
        )
        assertEquals(
            false,
            shouldUsePhotoStackBurn(
                ...
                stableScene = true,
                elapsedSincePreviousMs = PhotoStackRapidSkipWindowMs,
                randomValue = PhotoStackBurnProbability,
            ),
        )
    }
