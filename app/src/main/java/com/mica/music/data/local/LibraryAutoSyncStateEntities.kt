...
@Entity(tableName = "library_followup_outbox")
...
interface LibraryRetryItemDao {
    @Query(
        "SELECT * FROM library_retry_items WHERE source = :source AND stableIdentity = :stableIdentity",
    )
    suspend fun getBySource(source: String, stableIdentity: String): List<LibraryRetryItemEntity>
...
interface LibraryFollowupOutboxDao {
    @Query("SELECT * FROM library_followup_outbox ORDER BY createdAtMs, eventId")
    suspend fun getAll(): List<LibraryFollowupOutboxEntity>
