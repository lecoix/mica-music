    ...
    fun parse(text: String): List<LyricLine> {
        ...
        for (line in lines) {
            ...
            timedLine.findAll(trimmed).forEach { match ->
                ...
                if (body.isNotEmpty() && !LyricsSanitizer.isBinaryGarbage(body)) {
                    timed += LyricLine(
            ...
            if (!matchedAny && !trimmed.startsWith("[") && !LyricsSanitizer.isBinaryGarbage(trimmed)) {
        ...
        if (timed.isEmpty()) {
            val plain = text.lines()
                .map { it.trim() }
                .filter { it.isNotEmpty() && !tagLine.matches(it) && !LyricsSanitizer.isBinaryGarbage(it) }
