...
Service 的 USB output operation ID 区分每次异步等待、探测与权限请求；模式 `generation` 继续表示用户输出意图，native request epoch 继续表示底层会话所有权。新操作、阶段切换及 Service 销毁必须使旧 operation 失效，任何外部调用或延迟回调返回后都要在副作用前重新校验。`usbOutputDestroyed` 是每个 Service 实例的单向生命周期 token，并须具备跨线程可见性；`applyUsbOutputMode` / `dispatchUsbOutput` 统一 fail-close，已经排队的 preference、bootstrap、facts 与 audio-device callback 也必须在写共享字段前重验。USB 拔出事件仅在已知 `runtimeHandle` 匹配当前候选或 owner 目标时释放播放；目标未知时保留原有断开处理。USB 插入事件必须携带 `runtimeHandle` 与音频输出相关性；非音频设备不得启动探测，独占权限等待、打开或活动阶段只接受当前目标身份，不能因扩展坞上的无关设备重启独占链路。
