...
`LyricsPreferences`：歌词拆分、双语展示、字号与对齐、列表主题逐字动画预设、通知歌词开关与全局歌词偏移等。列表主题逐字动画预设只改变具有真实逐字时间轴的当前行渲染，默认保持音节抬升；`AppUiSettings`、`NotificationLyricsCoordinator` 与 `NotificationLyrics` 经此读写。歌词偏移的正数表示提前，有效值为全局层与 Room 单曲层相加；零偏移不含隐藏提前量，完整决策见 `docs/adr/0003-lyrics-timing-offset.md`。
...
`UsbOutputCoordinator` 是 USB 输出协调层唯一 owner：持有 reducer state、target/probe、shared-route wait、Android topology/permission/audio-device callback、owner facts、telemetry、operation fencing 与 effect 顺序；`MicaMediaService` 只通过 `UsbOutputCoordinator.start/submit/close` 和窄 `UsbOutputPlaybackPort` 参与。三种身份域必须严格分离：模式 `generation` 表示用户输出意图，`UsbOutputOperationId` 表示 probe / permission / timeout 尝试，`UsbRequestEpoch` 只由 `UsbHybridSessionOwner` mint 并表示 native session 所有权；不得用相同 `Long` 在这些域之间转换。Coordinator `close()` 必须先封死 Android/延迟回调并退役播放栈，再关闭 owner 与 Android/native adapter。USB 拔出事件仅在已知 `runtimeHandle` 匹配当前候选或 owner 目标时释放播放；目标未知时保留原有断开处理。USB 插入事件必须携带 `runtimeHandle` 与音频输出相关性；非音频设备不得启动探测，独占权限等待、打开或活动阶段只接受当前目标身份，不能因扩展坞上的无关设备重启独占链路。
...
`CUSTOM_STANDARD` 不进入该布局变形，而是复用粒子封面进入歌词云的横向整页滑动；播放页保持正常态，歌词目标页预先挂载。自定义标准打开竖屏队列时，封面从当前布局槽插值到歌词顶栏，不切到标准播放页。粒子封面打开竖屏队列时保持 GLES 层，只把封面槽缩到顶栏，不换成静态 `SongCover`，也不走歌词那种铺满全屏。拍立得打开竖屏队列时 Polaroid View 原位淡出（保持倾角与休息态尺寸），不转正、不飞顶栏；顶栏（含独立淡入的小专辑图）与列表另行淡入，标题留在下半屏原位一起淡出。
