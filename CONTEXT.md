...
远期独立输出路径：App 通过 Android USB Host 持有目标 USB audio interface，负责权限、claim、格式协商、传输与释放，并绕过系统共享 `AudioTrack`。Fosi SK02 的 USBFS + Media3 原型已收口；P1 已建立 production contract、正式 generation/session owner、typed Host request 与 output adapter seam，真机 open 已实际消费 exact-only 协商结果并在 claim 前验证 endpoint topology。P2 已接入 SK02 只读设备 snapshot、代际化 permission request/result、physical detach、独立中断播放意图、bounded recovery/SharedPcm fallback、process-restart durable intent 与本地播放 wake policy；旧权限、detach、health、recovery 和 fallback 回调不能覆盖当前事实。debug gate 可发起代际化 full-mode rebuild，候选 Exo stack 发布时原子迁移 MediaSession player、队列、索引、位置、repeat/shuffle、播放参数与播放意图，并重绑 player-scoped coordinators；过期候选只能释放，重建失败回滚 gate。release 仍默认关闭；双向 rebuild、权限拒绝重试、播放/暂停物理拔插、强杀恢复及短时 HOME/熄屏 smoke 已在 SK02 真机通过，完整 90 分钟 Continuous 与 Lifecycle 长测均已通过。长期 OEM 后台/系统回收、SharedPcm baseline、自动三连失败 fallback 注入、产品权限 UX/UI 与人工听感尚未完成。决策见 `docs/adr/0001-usb-host-exclusive-output.md`，证据与交接点见 `docs/USB_EXCLUSIVE_AUDIO_STATUS.md`。
