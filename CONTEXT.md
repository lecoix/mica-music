...
远期独立输出路径：App 通过 Android USB Host 持有目标 USB audio interface，负责权限、claim、格式协商、传输与释放，并绕过系统共享 `AudioTrack`。Fosi SK02 的 USBFS + Media3 原型已收口；P1 已建立 production contract、正式 generation/session owner、typed Host request 与 output adapter seam，真机 open 已实际消费 exact-only 协商结果并在 claim 前验证 endpoint topology。P2 已接入 SK02 只读设备 snapshot、代际化 permission request/result 与 physical detach 释放基础；旧权限回调和旧 runtime detach 不能覆盖当前事实。debug gate 现可发起代际化 full-mode rebuild，候选 Exo stack 发布时原子迁移 MediaSession player、队列、索引、位置、repeat/shuffle、播放参数与 `playWhenReady`，并重绑 player-scoped coordinators；过期候选只能释放，重建失败回滚 gate。release 仍默认关闭，短时生命周期 smoke 与一个完整 90 分钟 Continuous 长测已通过；新的 full-mode rebuild 尚待实体机 smoke，90 分钟 Lifecycle、SharedPcm baseline、人工听感以及 P2 的 durable recovery/前后台策略/UI 尚未完成。决策见 `docs/adr/0001-usb-host-exclusive-output.md`，证据与交接点见 `docs/USB_EXCLUSIVE_AUDIO_STATUS.md`。
